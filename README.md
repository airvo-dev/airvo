...
- ✅ **🧠 Fit LLM** — in Status, instantly see which Ollama models fit your current RAM/VRAM (`fits`, `tight`, `too_large`) with pull command and one-click add.
...
**5. Install continue.dev in VS Code**
...
| 🧠 **Fit LLM** | Status → System Resources now includes Fit LLM: it evaluates the Ollama catalog against your currently free RAM/VRAM and labels each model as `fits`, `tight`, or `too_large`. Includes `ollama pull` command and one-click add to Airvo. Endpoint: `GET /api/hardware/fit-models`. |
...
**⚡ Multi-Model Modes**
Run up to 5 models simultaneously in 4 modes: **Parallel** (see all answers), **Race** (fastest wins), **Vote** (consensus), **Review** (one generates, others refine).
...
**v0.9.5** — Fit LLM
- **Fit LLM** — `airvo/hardware/fit_models.py`. Evaluates Ollama catalog models against currently available RAM/VRAM and classifies each as `fits`, `tight`, or `too_large`. Exposed in Status → System Resources with summary chips, model list, pull command, and quick add flow.
- **New endpoint** — `GET /api/hardware/fit-models`.
- **Dashboard UI** — Fit LLM panel added to Status/Hardware in `dashboard/src/App.jsx`.
